...
const SearchWorkers = ({ workers }: SearchWorkersProps) => {
  return (
    <div className="flex flex-col gap-10 col-span-2 h-[4000px]">
      {workers.map((worker) => (
        <div key={worker.id} className="flex flex-col md:flex-row items-center justify-between gap-5 bg-cyan-50 hover:bg-cyan-100 transition cursor-pointer p-4">
          ...
          <Button className="text-lg lg:text-xl p-6 lg:p-10 w-full md:w-fit">Choose a date</Button>
        </div>
      ))}
    </div>
  );
